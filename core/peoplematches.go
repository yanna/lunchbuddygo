...
func (pm *PeopleMatches) GetPreferences(matchMode MatchMode) (group1Prefs map[string][]string, group2Prefs map[string][]string, oddPerson *Person) {
	group1, group2, oddPerson := pm.SplitActivePeopleIntoTwoRandomGroups()
	group1Prefs = pm.getPreferencesForFirstGroup(group1, group2, matchMode)
	group2Prefs = pm.getPreferencesForFirstGroup(group2, group1, matchMode)
	return
}
...
//If there is an odd number of people, an odd person may also be returned
func (pm *PeopleMatches) GetPreferencesParallel(matchMode MatchMode) (group1Prefs map[string][]string, group2Prefs map[string][]string, oddPerson *Person) {
	group1, group2, oddPerson := pm.SplitActivePeopleIntoTwoRandomGroups()

	group1PrefsChannel := make(chan map[string][]string)
	go pm.getPreferencesForFirstGroupParallel(group1, group2, group1PrefsChannel, matchMode)

	group2PrefsChannel := make(chan map[string][]string)
	go pm.getPreferencesForFirstGroupParallel(group2, group1, group2PrefsChannel, matchMode)
...
func (pm *PeopleMatches) getPreferencesForFirstGroup(group1 []Person, group2 []Person, matchMode MatchMode) map[string][]string {
	...
	for _, person := range group1 {
		group1Prefs[person.Alias] = pm.getPreferencesForPerson(person.ID, group2IDsSet, matchMode)
	}

	return group1Prefs
}

func (pm *PeopleMatches) getPreferencesForFirstGroupParallel(group1 []Person, group2 []Person, group1PrefsChannel chan map[string][]string, matchMode MatchMode) {
...
func (pm *PeopleMatches) getPreferencesForPerson(personID int, inputPersonIDs *Set, matchMode MatchMode) []string {
	...
	nonMatchedInInputSorted := pm.sortInOrderOfPreference(personID, nonMatchedInInput, matchMode)
...
func (pm *PeopleMatches) sortInOrderOfPreference(personID int, personIDsToSort []int, matchMode MatchMode) []int {
	...
	for i, personIDToSort := range personIDsToSort {
		...
		personScores[i] = PersonScore{
			...
			score:    personToSort.GetScore(&person, matchMode),
		}
	}
